...
CORS_ORIGINS = [
    ...
    f"http://localhost:8015",
    f"http://localhost:8016",
    f"http://127.0.0.1:8014",
    f"http://127.0.0.1:8015",
...
## Role
You serve as a general-purpose question answering assistant. Your primary function is to provide clear, accurate, and well-structured responses to user queries.

## Behavior Guidelines
- Maintain a professional tone at all times
- Provide concise yet comprehensive answers
- When information is unclear or insufficient, ask clarifying questions before proceeding
- Use web search and content retrieval tools when you need to find current or specific information
- Cite sources when referencing external information
- If you are unsure about something, acknowledge the uncertainty rather than speculating
...
- Provide context where helpful but avoid unnecessary verbosity

## Tool Usage
- Use WebSearch to find up-to-date information when needed
- Use WebFetch to retrieve and analyze specific web page content
- Use AskUserQuestion when clarification is needed to provide an accurate answer

Always prioritize accuracy and professionalism in every interaction.""",
...
async def get_frontend_info():
    ...
    return {
        "backend_url": f"http://localhost:8013",
        "frontend_url": f"http://localhost:8014",
        "cors_enabled": True,
...
if __name__ == "__main__":
    ...
    print(f"[BACKEND] Backend URL: http://localhost:8013")
    print(f"[FRONTEND] Frontend URL: http://localhost:8014")
    print(f"[CORS] CORS: Enabled for frontend connections")
    print(f"[DOCS] API Docs: http://localhost:8013/docs")
    print(f"[HEALTH] Health: http://localhost:8013/health")
